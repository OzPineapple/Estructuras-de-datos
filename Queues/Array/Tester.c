...
int main( int argc, char * argv[] ){
	...
	for( size_t index = 0; index < randomInRange( 1,5 ); index++)
		writeOnQueueArray( queue_array, randomInRange( 0, queue_array -> length - 1 ), randomInRange( -100, 100 ) );
	printQueueArray( queue_array );
	println;

	printf( "\nQueueArray.h - Read\n" );
	for( size_t index = 0; index < randomInRange( 1,5 ); index++)
		printf( "[%i]",	readOnQueueArray( queue_array, randomInRange( 0, queue_array -> length - 1 ) ) );
	printf( "\n" );
	printQueueArray( queue_array );
	println;
