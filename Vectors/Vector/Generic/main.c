...
main (int argc, char *argv[])
{
  ...
  initGenericVectorPointer (&generic_vector, randomInRange (1, 80),
			    sizeof (float));
  ...
  printFromGenericVector (generic_vector,
			  randomInRange (0, generic_vector->length - 1),
			  "%i\n", int);

  writeOnGenericVector (generic_vector,
			randomInRange (0, generic_vector->length - 1),
			randomInRange (1, 10), int);
  printGenericVector (generic_vector, "[%i]", int);
