...
int *
resizeArray (int *array, int size, int newSize)
{
  valPtr (array);
  valSize (size);
  valSize (newSize);
...
//Lookers
void
seeArray (int *array, int size, char * *arrayPhotography,
  ...
  for (int i = 0; i < size; i++)
    {
      ...
      *arrayPhotography =
	(char *) realloc (*arrayPhotography,
			  sizeof (char) * (*arrayPhotographySize));
      valPtr (*arrayPhotography);
...
int
readOnArray (int *array, int size, int pos)
{
  valPtr (array);
  valSize (size);
  valNeg (pos);
...
void
printFromArray (int *array, int size, int pos)
{
  valPtr (array);
  valSize (size);
  valNeg (pos);
...
//Modifyers
void
writeOnArray (int *array, int size, int pos, int data)
{
  valPtr (array);
  valSize (size);
  valNeg (pos);
...
void
deleteOnArray (int *array, int size, int pos)
{
  valPtr (array);
  valSize (size);
  valNeg (pos);
...
void
deleteArray (int *array, int size)
{
  valPtr (array);
  valSize (size);
...
void
cloneArray (int *originalArray, int *cloneArray, int size)
{
