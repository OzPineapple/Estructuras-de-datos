...
QueueArray_t *
destroyQueueArray (QueueArray_t * queue)
{
  if (queue == 0x0)
    return 0x0;
...
void
printQueueArray (QueueArray_t * queue)
{
  if (queue == 0x0)
    {
      printf ("(nil)");
...
int
readOnQueueArray (QueueArray_t * queue, size_t pos)
{
  valPtr (queue);
...
QueueArray_t *
queueArrayPush (QueueArray_t * queue, int value)
{
  if (queue == 0x0)
    {
...
int
queueArrayPop (QueueArray_t * queue)
{
  if (queue->array == 0x0)
    {
      return 0;
  ...
  for (size_t i = 0, j = 1; j < queue->length; i++, j++)
    {
      writeOnArray (queue->array, queue->length, i,
		    readOnArray (queue->array, queue->length, j));
    }
...
void
writeOnQueueArray (QueueArray_t * queue, size_t pos, int value)
{
  valPtr (queue);
  valSizePos (queue->length, pos);
