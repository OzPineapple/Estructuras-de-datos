...
void
initNode (Node_t * node, int value, int length)
{
  valSize (length);
  node->value = value;
  node->length = length;
...
void
initNodePointer (Node_t * *node, int value, int length)
{
...
seeNode (Node_t * node, char * *nodePhotography, int *nodePhotographySize)
{
  ...
  *(nodePhotographySize) =
    snprintf (0x0, 0, photography, (Node_t *) node, node->value,
	      node->length) + 1;
  *(nodePhotography) =
    (char *) calloc (*(nodePhotographySize), sizeof (char));
  sprintf (*(nodePhotography), photography, (Node_t *) node, node->value,
	   node->length);
  ...
  for (int i = 0; i < node->length; i++)
    {
      elementPhotographySize =
	snprintf (0x0, 0, "[%p]",
		  (Node_t *) * ((Node_t * *)(node->way + i))) + 1;
      elementPhotography =
	(char *) calloc (elementPhotographySize, sizeof (char));
      valMem (elementPhotography);
      sprintf (elementPhotography, "[%p]", *((Node_t * *)(node->way + i)));

      *(nodePhotographySize) += elementPhotographySize;
      *(nodePhotography) =
	(char *) realloc (*(nodePhotography),
			  sizeof (char) * *(nodePhotographySize));
...
void
printNodes (size_t count, Node_t * node, ...)
{
  va_list args;
  va_start (args, node);
...
/* Modify  */
void
changeNodeValue (Node_t * node, int value)
{
  node->value = value;
}

void
deleteNodeValue (Node_t * node)
{
  changeNodeValue (node, 0);
}

void
writeOnNodeWay (Node_t * node, int pos, Node_t * link)
{
...
void
deleteOnNodeWay (Node_t * node, int pos)
{
  writeOnNodeWay (node, pos, 0x0);
}

void
resizeNodeWay (Node_t * node, int length)
{
  valSize (length);
...
void
linkNode (size_t way, Node_t * node_one, Node_t * node_two)
{
  valNeg (way);
  valPtr (node_one);
...
void
linkNodes (size_t way, size_t count, Node_t * node, ...)
{
  valNeg (way);
...
void
unlinkNode (size_t way, Node_t * node_one)
{
  valNeg (way);
  valPtr (node_one);
  valSizePos (node_one->length, way);
