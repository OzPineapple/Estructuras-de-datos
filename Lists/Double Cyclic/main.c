...
main (int argc, char *argv[])
{
  ...
  printNode (getNodeOnListCyclicDouble
	     (list_cyclic_double,
	      randomInRange (0,
			     lengthOfListCyclicDouble (list_cyclic_double) -
			     1)));

  printf ("\nListCyclicDouble.h - Write\n");

  for (size_t i = 0; i < 10; i++)
    writeOnListCyclicDouble (list_cyclic_double,
			     randomInRange (0,
					    lengthOfListCyclicDouble
					    (list_cyclic_double) - 1),
  ...
  for (size_t i = 0; i < 10; i++)
    printf ("%i ",
	    readOnListCyclicDouble (list_cyclic_double,
				    randomInRange (0,
						   lengthOfListCyclicDouble
						   (list_cyclic_double) -
						   1)));
  ...
  for (size_t i = 0; i < 10; i++)
    deleteOnListCyclicDouble (list_cyclic_double,
			      randomInRange (0,
					     lengthOfListCyclicDouble
					     (list_cyclic_double) - 1));
  ...
  printListCyclicDouble (list_cyclic_double);
  if (list_cyclic_double != 0x0)
    destroyNodeOnListCyclicDoubleNlink (&list_cyclic_double,
					lengthOfListCyclicDouble
					(list_cyclic_double) - 1);
  printListCyclicDouble (list_cyclic_double);
  if (list_cyclic_double != 0x0)
