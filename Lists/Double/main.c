...
main (int argc, char *argv[])
{
  ...
  printNode (getNodeOnListDouble
	     (list_double,
	      randomInRange (0, lengthOfListDouble (list_double) - 1)));

  printf ("\nListDouble.h - Write\n");

  for (size_t i = 0; i < 10; i++)
    writeOnListDouble (list_double,
		       randomInRange (0,
				      lengthOfListDouble (list_double) - 1),
  ...
  for (size_t i = 0; i < 10; i++)
    printf ("%i ",
	    readOnListDouble (list_double,
			      randomInRange (0,
					     lengthOfListDouble (list_double)
					     - 1)));
  printf ("\n");
  ...
  for (size_t i = 0; i < 10; i++)
    deleteOnListDouble (list_double,
			randomInRange (0,
				       lengthOfListDouble (list_double) - 1));
  ...
  destroyNodeOnListDoubleNlink (&list_double,
				randomInRange (0,
					       lengthOfListDouble
					       (list_double) - 1));
