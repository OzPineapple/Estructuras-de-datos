...
void
initListDoublePointer (Node_t * *list, size_t size)
{
  valSize (size);
  *list = newList (size, 2);
  valMem (*list);
...
void
listDoubleInsert (Node_t * *list, Node_t * node)
{
  val2Ptr ((void **) list);
  valPtr (node);
...
void
listDoubleAppend (Node_t * list, Node_t * node)
{
  valPtr (list);
...
Node_t *
popFirstNodeOnListDoubleNlink (Node_t * *list)
{
  val2Ptr ((void **) list);
  Node_t *second = getNextNodeOnList (0, *list);
  unlinkNode (1, second);
...
Node_t *
popNodeOnListDoubleNlink (Node_t * *list, size_t pos)
{
  val2Ptr ((void **) list);
  valNeg (pos);
...
Node_t *
popLastNodeOnListDoubleNlink (Node_t * list)
{
  Node_t *last = getLastNodeOnList (0, list);
...
void
destroyNodeOnListDoubleNlink (Node_t * *list, size_t pos)
{
  val2Ptr ((void **) list);
  valNeg (pos);
